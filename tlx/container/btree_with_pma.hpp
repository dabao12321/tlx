...
#ifndef TLX_CONTAINER_BTREE_PMA
#define TLX_CONTAINER_BTREE_PMA
...
private:
    ...
    struct LeafNode : public node {
        ...
        //! True if few used entries, less than half full.
        bool is_few() const {
						return slotdata.get_num_elts() <= leaf_slotmin;
        }
...
private:
    ...
    unsigned short find_lower(const node_type* n, const key_type& key) const {
				if constexpr (std::is_same_v<node_type, InnerNode>) {
...
public:
    ...
    iterator find(const key_type& key) {
        ...
        return (slot < leaf_slotmax && key_equal(key, leaf->slotdata.blind_read_key(slot)))
    ...
    */
    std::pair<iterator, bool> insert_descend(
            ...
            if (newchild)
            {
                ...
                if (inner->is_full())
                {
                    ...
                    if (slot == inner->slotuse + 1 &&
                        inner->slotuse < ((InnerNode*)(*splitnode))->slotuse)
                    {
        ...
        else // n->is_leafnode() == true
        {
            ...
            if (splitnode && leaf != *splitnode && key == leaf->slotdata.get_max()) 
						// slot == leaf->slotuse - 1)
            {
    ...
    void verify_node(const node* n, key_type* minkey, key_type* maxkey,
        ...
        if (n->is_leafnode())
        {
            ...
            tlx_die_unless(leaf->slotuse > 0);
						...
						// TODO: Write a verify_pma that checks that the elts are increasing
